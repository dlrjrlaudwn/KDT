...
ss_per=ss_cash[26]
ss_pbr=ss_cash[28]
ss_cash_flow=ss_cash2.find_all('span',{'class':'cUp'})
...
ss=pd.DataFrame([ss_c_dict,ss_r_dict,ss_p_dict,ss_pbr_dict],index=['투자활동현금흐름','ROE','PER','PBR'])
ss.loc['투자활동현금흐름']=ss.loc['투자활동현금흐름'].str.replace(',','')
...
def plot_maker2(x,y,title,color):
    ...
    for i in range(len(x)):
        height = y[i]
        plt.text(x[i], height + 0.05, '%.0f' %height, ha='center', va='bottom', size = 12)
    current_values = plt.gca().get_yticks()
    plt.gca().set_yticklabels(['{:,.0f}'.format(x) for x in current_values])
    plt.tight_layout()
    plt.show()
...
plot_maker(ss.index,ss.iloc[:,0],'투자활동현금흐름','#FF7AA2')
