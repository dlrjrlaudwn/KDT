...
inner join author_isbn on authors.author_id=author_isbn.author_id
order by lastname,firstname;

# 7
select aisbn.author_id,t.isbn,t.title,t.edition_number,t.copyright from titles as t
inner join author_isbn as aisbn on t.isbn=aisbn.isbn
order by isbn desc;

# 8
select a.firstname, a.lastname, t.title, aisbn.isbn, t.copyright from titles t
inner join author_isbn aisbn
on t.isbn=aisbn.isbn
inner join authors a
on a.author_id=aisbn.author_id
...
inner join authors a
on a.author_id=aisbn.author_id
where (firstname='Paul' and lastname='Deitel') or (firstname='Harvey' and lastname='Deitel');

# 10
select t.title, aisbn.isbn, t.copyright from titles t
inner join author_isbn aisbn
on t.isbn=aisbn.isbn
inner join authors a
on a.author_id=aisbn.author_id
